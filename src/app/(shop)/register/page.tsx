...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-screen flex">
      {/* Left Hero Panel */}
      <div className="hidden lg:flex lg:w-1/2 relative bg-gradient-to-br from-emerald-900 via-emerald-800 to-teal-900 items-center justify-center overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-10 left-10 w-72 h-72 bg-white rounded-full blur-3xl" />
          <div className="absolute bottom-10 right-10 w-96 h-96 bg-teal-300 rounded-full blur-3xl" />
          <div className="absolute top-1/3 right-1/4 w-48 h-48 bg-emerald-300 rounded-full blur-3xl" />
        </div>
        <div className="relative z-10 text-center px-12 max-w-lg">
          ...
          <h2 className="text-3xl font-bold text-white mb-4">Join the Family</h2>
          <p className="text-emerald-200 text-sm leading-relaxed mb-8">
            Become a member and enjoy exclusive offers, early access to fresh catches, and seamless ordering.
          </p>
            ...
            {['🎁', '🚚', '⭐'].map((emoji, i) => (
              <div key={i} className="bg-white/10 backdrop-blur-sm rounded-xl py-4 px-3 ring-1 ring-white/15">
                ...
                <div className="text-[11px] text-emerald-200 font-medium">
                  {['Exclusive Offers', 'Free Delivery', 'Rewards'][i]}
                </div>
              </div>
            ))}
          </div>
